...
{
    packet_with_id(Packet::Ptr p, entity_id_t i) :
        packet(p),
        id(i)
    {}
...
template <class Allocator = std::allocator<detail::pending_data<packet_with_id>>>
class most_recent_packer_with_id : protected packer<most_recent_packer_with_id<A>, packet_with_id, Allocator>
{
    template <class A> friend class packer<most_recent_packer_with_id, packet_with_id, A>;

public:
    using packer_t = packer<most_recent_packer_with_id, packet_with_id, Allocator>;

public:
    using packer<most_recent_packer_with_id<A>, packet_with_id, Allocator>::packer;
...
template <typename T, typename... Args>
void most_recent_packer_with_id<Allocator>::add(rpc::Opcode opcode, T&& data, Args&&... args)
{
...
template <class Allocator>
void most_recent_packer_with_id<Allocator>::add(const Packet::Ptr& packet, entity_id_t id)
{
    // Add to pending
...
template <class Allocator>
inline void most_recent_packer_with_id<Allocator>::on_ack(const pending_vector_t::iterator& part)
{
...
template <class Allocator>
void most_recent_packer_with_id<Allocator>::process(uint16_t block_id, uint16_t& remaining, detail::packets_by_block& by_block)
{
...
template <class Allocator>
inline void most_recent_packer_with_id<Allocator>::clear()
{
    _id_map.clear();
}
