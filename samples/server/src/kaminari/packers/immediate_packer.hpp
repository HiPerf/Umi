...
template <class Allocator = std::allocator<detail::pending_data<packet::ptr>>>
class immediate_packer : protected packer<immediate_packer, packet::ptr, Allocator>
{
    template <class A> friend class packer<immediate_packer<A>, packet::ptr, A>;

public:
    using packer_t = packer<immediate_packer, packet::ptr, detail::pending_data<packet::ptr>>;

public:
    using packer<immediate_packer, packet::ptr, Allocator>::packer;
...
template <typename T, typename... Args>
void immediate_packer<Allocator>::add(rpc::Opcode opcode, T&& data, Args&&... args)
{
...
template <class Allocator>
void immediate_packer<Allocator>::add(const packet::ptr& packet)
{
    // Add to pending
...
template <class Allocator>
void immediate_packer<Allocator>::process(uint16_t block_id, uint16_t& remaining, detail::packets_by_block& by_block)
{
...
template <class Allocator>
inline void immediate_packer<Allocator>::on_ack(const pending_vector_t::iterator& part)
{
    // Nothing to do here
    (void)part;
}
...
template <class Allocator>
inline void immediate_packer<Allocator>::clear()
{}
